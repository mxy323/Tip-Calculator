...
import "../styles/inputStyle.less";
import { Digit } from "../types";
//import dollar from "../asserts/icon-dollar.svg";

function InputPart() {
  const [bill, setBill] = useState(0);
  const [isSelectTip, setIsSelectTip] = useState(false);
  ...
  const getBillNum = (e: React.ChangeEvent<HTMLInputElement>) => {
    //限制输入类型；
  ...
  return (
    <div className="input-part">
      ...
      <div className="tip">
        {/* 遍历 */}
        <button className="tips tip-5" onClick={() => onDigitButtonClick(0.05)}>
          5%
        </button>
        <button className="tips tip-5" onClick={() => onDigitButtonClick(0.1)}>
          10%
        </button>
        <button className="tips tip-5" onClick={() => onDigitButtonClick(0.15)}>
          15%
        </button>
        <button className="tips tip-5" onClick={() => onDigitButtonClick(0.25)}>
          25%
        </button>
        <button className="tips tip-5" onClick={() => onDigitButtonClick(0.5)}>
          50%
        </button>
        <div className="tips tip-custom">
      ...
      <div className="people-num">
        ...
        <input className="num" type="number" placeholder="0" min="0" />
      </div>
    </div>
  );
}
